...
def handler(request):
    if not SMTP_USER or not SMTP_PASSWORD:
        return _json({"ok": False, "error": "SMTP non configuré côté serveur."}, 500)
    ...
    if errors:
        return _json({"ok": False, "errors": errors}, 422)

    try:
        _send_email(name, email, subject, message)
    except Exception:
        return _json({"ok": False, "error": "Envoi impossible. Réessayez plus tard."}, 500)

    return _json({"ok": True, "message": "Message envoyé."}, 200)
